...
class Pump2000: #defines Pump2000 class with serial connection, address, and name (used in logging)
    ...
    def set_syringe_volume(self,syringe_volume,syringe_volume_units,bytes=60):#sets the syringe volume for a pump
        if(syringe_volume_units == 'ul'): # need to convert syringe volume (from ul to ml) if syringe volume unit is ul 
...
if __name__ == '__main__':
    ...
    group.add_argument('-infuse_wait', action="store_true", help = 'set pump to infuse and wait until target volume has been reached')
    group.add_argument('-withdraw_wait', action="store_true", help = 'set pump to infuse and wait until target volume has been reached')
    group.add_argument('-poll', action="store_true", help = 'check if pump has reached target volume')
    group.add_argument('-poll_infuse', action="store_true", help = 'check if pump has reached target volume (after infusing)')
    group.add_argument('-poll_withdraw', action="store_true", help = 'check if pump has reached target volume (after withdrawing)')
    ...
    try:
        if args.PHDULTRA: #if PHDULTRA is an argument from the command line 
            ...
            if args.stop: # check if stop is an argument
                ...
                if args.infuse_wait:
                    iw = pump_ultra.wait_for_target(i_or_w="infuse",bytes=60) #runs pump in the infuse direction then waits for target 
                if args.withdraw_wait:
                    ww = pump_ultra.wait_for_target(i_or_w="withdraw",bytes=60) #runs pump in the infuse direction then waits for target 
